...
voltInterval evaluateSwitch(unsigned int voltage){
  voltInterval returnInterval = UNDIFINED_INTERVAL; //check, if NULL is ok as default - what happens if NULL is returned?
  if((voltage >= 0) && (voltage <= 125)){
    returnInterval = FAIL_GND_SHORT;
  } // 0 to 0,1V 
  else if((voltage >= 370) && (voltage <= 625)){
    returnInterval = SWITCH_LOW_VAL;
  } // 0,3 to 0,5V 
  else if((voltage >= 805) && (voltage <= 935)){
    returnInterval = FAIL_IGNORE;
  } // 0,65 to 0,75V 
  else if((voltage >= 990) && (voltage <= 1120)){
    returnInterval = FAIL_IGNORE;
  } // 0,80 to 0,90V 
  else if((voltage >= 1240) && (voltage <= 1490)){
    returnInterval = SWITCH_NORM_VAL;
  } // 1,00 to 1,20
  else if((voltage >= 1920) && (voltage <= 2110)){
    returnInterval = FAIL_OPEN;
  } // 1,55 to 1,70
    ...
    returnInterval = SWITCH_HIGH_VAL;
  } // 2,46 to 2,60 
  else if((voltage >= 3475) && (voltage <= 3540)){
    returnInterval = FAIL_IGNORE;
  } // 2,80 to 2,85
  else if((voltage >= 3720) && (voltage <= 4095)){
    returnInterval = FAIL_VCC_SHORT;
  } // 3,00 to 3,30
  else {
    Serial.println("Error: Could not map measured Voltage at Switch-Senses to one of the expected intervals.");
  }
 return returnInterval;  //default: UNDEFINED_INTERVAL 
}
...
systemState analyseSystemState(voltInterval switchStatus1, voltInterval switchStatus2/*, unsigned int switch1, unsigned int switch2*/){
  ...
  //HW-Failures
    ...
    Serial.println("Warning: Detected unsynchronized-behaviour of the Switch Button and ignored it.");
  }
  else{
      // Unknown combination of voltage-intervals - or lack of voltage interval(s)
      Serial.println("Error: Could not map the measured Voltages to an expected systemState.");
  }
  return sysState; //default: UNDEFINED_STATE
